...
export function Page() {
  ...
  const delegateAddress = userResp?.data?.address as `0x${string}`;
  const starknetAddress = userResp?.data?.starknetAddress as `0x${string}`;
  ...
  const hasDelegatedOnL2 =
    ...
    userResp?.data?.starknetAddress?.toLowerCase();
  const hasDelegatedOnL1 =
    delegationDataL1 &&
    delegationDataL1.length &&
    delegationDataL1.toLowerCase() === userResp?.data?.address?.toLowerCase() &&
          ...
          <>
            <AvatarWithText
              ...
              src={
                ...
                userResp?.data?.ensAvatar ||
                null
              }
              headerText={
                userResp?.data?.username ||
                userResp?.data?.ensName ||
              ...
              headerTooltipContent={
                !userResp?.data?.username && !userResp?.data?.ensName
                  ? delegateAddress
                  : undefined
              }
        ...
        <Box mt="standard.2xl" pb="standard.2xl">
          <SummaryItems.Root>
            ...
            <SummaryItems.Item
              ...
              isCopiable={!!userResp?.data?.starknetAddress}
              isTruncated={!!userResp?.data?.starknetAddress}
              label="Starknet address"
              value={userResp?.data?.starknetAddress || "None"}
              additionalValue={votingPowerL2?.toString() || "0"}
              delegatedToValue="cillian.eth"
              isExtendable={true}
            />
            <SummaryItems.Item
              isLoading={!delegateResponse.isFetched}
              isCopiable={!!userResp?.data?.address}
              isTruncated={!!userResp?.data?.address}
              label="Ethereum address"
              value={userResp?.data?.address || "None"}
              additionalValue={votingPower?.toString() || "0"}
              delegatedToValue="dawae"
              isExtendable={true}
            />
            <SummaryItems.Item
