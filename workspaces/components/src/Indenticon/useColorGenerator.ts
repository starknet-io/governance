import { useState, useEffect } from "react";
import ColorHash from "color-hash";

const colorHash = new ColorHash({ saturation: 1.0 });

const stringToColour = (s: string) => colorHash.hex(s);

const generateColours = (s: string) => {
  const s1 = s.substring(0, s.length / 2);
  const s2 = s.substring(s.length / 2);
  const c1 = stringToColour(s1);
  const c2 = stringToColour(s2);

  return [c1, c2];
};

const generateSVG = (s: string, size = 48) => {
  const [c1, c2] = generateColours(s);

  const svg = `
<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" fill="none" xmlns="http://www.w3.org/2000/svg">
  <circle cx="${size / 2}" cy="${size / 2}" r="${
    size / 2
  }" fill="url(#gradient-${s})" />
  <defs>
    <linearGradient id="gradient-${s}" x1="0" y1="0" x2="${size}" y2="${size}" gradientUnits="userSpaceOnUse">
      <stop stop-color="${c1}" />
      <stop offset="1" stop-color="${c2}" />
    </linearGradient>
  </defs>
</svg>
  `.trim();

  return svg;
};

export const useColorGenerator = (s: string, size: number) => {
  const [color, setColor] = useState("");
  const [svg, setSvg] = useState("");

  useEffect(() => {
    setColor(stringToColour(s));
    setSvg(generateSVG(s, size));
  }, [s]);

  return { color, svg };
};
